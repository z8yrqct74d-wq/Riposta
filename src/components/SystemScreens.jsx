...
function RoleAwareLanding({ role }) {
  ...
  const map = {
    admin:   { label: 'Admin',   dest: '/admin',   desc: 'Opening Admin console…' },
    coach:   { label: 'Coach',   dest: '/coach',   desc: 'Opening Coach console…' },
    athlete: { label: 'Athlete', dest: '/athlete', desc: 'Opening Athlete app…' },
  };
  const m = map[role] || map.athlete;
  React.useEffect(() => {
    const t = setTimeout(() => navigate(m.dest), 1800);
          ...
          <div style={{ animation: 'r-rise var(--d-slow) var(--e-enter) both' }}>
            ...
            <div style={{ marginTop: 32, padding: '14px', background: 'var(--surface)', border: '1px solid var(--hairline)', borderRadius: 'var(--r-card)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[['Admin','admin@riposte.ro'],['Coach','sandu@riposte.ro'],['Athlete','maya@riposte.ro']].map(([r, em]) => (
                  <button key={r} onClick={() => { setEmail(em); setTimeout(() => arrive(r.toLowerCase()), 200); }} className="r-focusable" style={{ font: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 'var(--r-btn)', border: '1px solid var(--hairline)', background: 'var(--paper)', fontSize: 13.5, color: 'var(--ink)', fontWeight: 500 }}>
                    <span style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--steel-tint)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--steel)', flexShrink: 0 }}>{r[0]}</span>
                    {r} <span style={{ color: 'var(--faint)', fontSize: 12, marginLeft: 4 }}>{em}</span>
                  </button>
                ))}
              </div>
            </div>
