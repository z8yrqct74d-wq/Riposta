// ============================================================
// RIPOSTE — Coach console (dark) · My day (B1), availability (B4), shell
// ============================================================

// local kind→bar (coach console doesn't load admin-data)
...
// ---- Month data (June 2026, 1-indexed days) ----
const MONTH_SESSIONS = {
  2: [{ type: 'lesson' }],
  3: [{ type: 'lesson' }, { type: 'group' }],
  4: [{ type: 'group' }, { type: 'lesson' }],
  5: [{ type: 'group' }, { type: 'group' }, { type: 'lesson' }],
  6: [{ type: 'lesson' }],
  7: [{ type: 'group' }],
  9: [{ type: 'lesson' }],
  10: [{ type: 'group' }, { type: 'lesson' }],
  11: [{ type: 'group' }],
  12: [{ type: 'lesson' }, { type: 'lesson' }],
  14: [{ type: 'group' }],
  16: [{ type: 'lesson' }],
  17: [{ type: 'group' }, { type: 'lesson' }],
  18: [{ type: 'group' }],
  19: [{ type: 'lesson' }],
  21: [{ type: 'group' }, { type: 'lesson' }],
  23: [{ type: 'lesson' }],
  24: [{ type: 'group' }],
  25: [{ type: 'lesson' }, { type: 'lesson' }],
  26: [{ type: 'group' }],
  28: [{ type: 'lesson' }],
  30: [{ type: 'group' }, { type: 'lesson' }],
};
  ...
  return (
    <div className="r-scroll" style={{ overflowX: 'auto', paddingBottom: 100 }}>
      <div style={{ display: 'flex', gap: 8, padding: '8px 16px', minWidth: 480 }}>
        {WEEK_DAYS.map((d, di) => (
          <div key={d.dow} style={{
            flex: d.today ? 1.15 : 1, minWidth: 0,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {d.items.length === 0 ? (
                ...
                return (
                  <button key={ii} onClick={() => d.today && onOpen && onOpen(it)} className="r-focusable" style={{
                    font: 'inherit', cursor: d.today ? 'pointer' : 'default', textAlign: 'left', border: 'none',
                    background: 'var(--surface)', borderLeft: `3px solid ${bar}`, borderRadius: 'var(--r-cal)',
                    padding: '6px 7px', opacity: it.status === 'done' ? 0.45 : 1,
...
function MonthView() {
  const [selDay, setSelDay] = React.useState(5); // today
  // June 2026 starts on Monday (day-of-week 1)
  const firstDow = 0; // 0=Mon offset (June 1 is a Monday)
    ...
    <div className="r-scroll" style={{ overflowY: 'auto', paddingBottom: 100 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3, padding: '0 12px' }}>
        {cells.map((d, i) => {
          ...
          return (
            <button key={d} onClick={() => setSelDay(d)} className="r-focusable" style={{
              font: 'inherit', cursor: 'pointer', border: 'none', borderRadius: 8,
              padding: '6px 4px 5px',
        ...
        {selSessions.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {selSessions.map((s, i) => {
              ...
              const items = selDay === 5 ? MY_DAY.filter(m => m.type === s.type) : [];
          ...
          <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--hairline)', borderRadius: 'var(--r-card)', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, animation: `r-rise var(--d-base) var(--e-enter) ${i*35}ms both` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 5, alignItems: 'center' }}>
                ...
                {a.streak > 0 && (
                  <span style={{ fontSize: 11, color: 'var(--steel)' }}>{a.streak}wk streak</span>
                )}
                ...
                {DAYS.map(d => {
                  ...
                  return (
                    <button key={d+s} disabled={blackout[d]} onClick={() => setGrid(g => ({ ...g, [`${d}|${s}`]: !g[`${d}|${s}`] }))} className="r-focusable" style={{
                      font: 'inherit', cursor: blackout[d] ? 'default' : 'pointer', aspectRatio: '1', borderRadius: 6,
                      border: '1px solid ' + (on ? 'var(--brand)' : 'var(--hairline)'),
                      background: blackout[d] ? 'repeating-linear-gradient(45deg, transparent, transparent 4px, var(--hairline) 4px, var(--hairline) 5px)' : on ? 'var(--brand)' : 'var(--surface)',
...
function StubScreen({ title }) {
  return (
    <div style={{ height: '100%', background: 'var(--paper)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 40 }}>
      <Icon name="users" size={36} color="var(--hairline)" />
      <div className="r-display" style={{ fontSize: 22, color: 'var(--ink)', margin: '16px 0 6px' }}>{title}</div>
      <div style={{ fontSize: 13.5, color: 'var(--muted)', maxWidth: 240, lineHeight: 1.5 }}>Themed and mapped — lands in the next wave.</div>
    </div>
  );
}
...
function CoachApp() {
  ...
  const [stack, setStack] = React.useState(null); // 'session' | 'lesson'
