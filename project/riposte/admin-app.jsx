// ============================================================
// RIPOSTE — Admin console · shell, nav, AI digest, side panel, toast
// ============================================================
...
function selStyle() {
  return { width: '100%', padding: '9px 11px', borderRadius: 'var(--r-btn)', border: '1px solid var(--hairline)', background: 'var(--paper)', font: 'inherit', fontSize: 13.5, color: 'var(--ink)', boxSizing: 'border-box' };
}
...
function Toast({ toast }) {
  ...
  return (
    <div style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 80, background: 'var(--surface)', border: `1px solid ${tone[0]}`, borderLeft: `3px solid ${tone[0]}`, borderRadius: 'var(--r-btn)', padding: '11px 16px', boxShadow: 'var(--shadow-raise)', display: 'flex', alignItems: 'center', gap: 9, animation: 'r-toast 200ms var(--e-enter)' }}>
      ...
      <style>{`@keyframes r-toast { from { opacity: 0; transform: transl(-50%, 8px); } to { opacity: 1; transform: translateX(-50%); } }`}</style>
    </div>
  );
}
