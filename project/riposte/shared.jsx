// ============================================================
// RIPOSTE — shared primitives (loaded by all three surfaces)
// Weapon glyphs, line icons, avatars, status pills/badges.
// Exports to window for cross-script use.
// ============================================================

// ---- Weapon glyphs ----------------------------------------------------
// Custom, consistent set. Blade runs lower-left → upper-right.
//  foil  = thin straight blade, small ring guard
//  épée  = thicker blade, deep bell guard + fuller line
//  sabre = curved blade, knuckle-bow guard
function WeaponGlyph({ type = 'foil', size = 20, color = 'var(--steel)', strokeWidth, style = {} }) {
...
const WEAPON_LABEL = { foil: 'Foil', epee: 'Épée', sabre: 'Sabre' };
...
function Avatar({ name = '', size = 32, src, style = {} }) {
...
// ---- Payment status pill ----------------------------------------------
function PaymentPill({ status, size = 'md' }) {
  const map = {
...
// ---- Weapon chip (glyph + label) --------------------------------------
function WeaponChip({ type, showLabel = true }) {
  return (
...
Object.assign(window, {
  WeaponGlyph, WEAPON_LABEL, Icon, Avatar, PaymentPill, VisaBadge, WeaponChip,
});
