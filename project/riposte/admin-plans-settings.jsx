// ============================================================
// RIPOSTE — Admin · A6 Plans & billing + A7 Settings
// ============================================================
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ borderBottom: '1px solid var(--hairline)', background: 'var(--surface)', display: 'flex', padding: '0 24px' }}>
        {tabs.map(([id, label]) => (
          <button key={id} onClick={() => setView(id)} className="r-focusable" style={{ font: 'inherit', cursor: 'pointer', border: 'none', background: 'transparent', padding: '12px 16px', fontSize: 13.5, fontWeight: view===id ? 600 : 500, color: view===id ? 'var(--ink)' : 'var(--muted)', borderBottom: '2px solid ' + (view===id ? 'var(--brand)' : 'transparent'), transition: 'color var(--d-fast)', whiteSpace: 'nowrap' }}>{label}{id==='dunning'&&DUNNING.length>0&&<span style={{ marginLeft: 7, background: 'var(--danger)', color: '#fff', fontSize: 10, fontWeight: 700, borderRadius: 'var(--r-pill)', padding: '1px 6px' }}>{DUNNING.length}</span>}</button>
        ))}
      </div>
              ...
              <tbody>
                {SUBS.map((s, i) => (
                  <tr key={i} style={{ borderBottom: i < SUBS.length-1 ? '1px solid var(--hairline)' : 'none', animation: `r-rise var(--d-base) var(--e-enter) ${i*30}ms both` }}>
                    <td style={{ padding: '11px 16px', display: 'flex', alignItems: 'center', gap: 10 }}><Avatar name={s.name} size={28} /><span style={{ fontWeight: 500, color: 'var(--ink)' }}>{s.name}</span></td>
...
Object.assign(window, { AdminPlans, AdminSettings });
