// ============================================================
// RIPOSTE — Coach console (dark) · Session attendance + Lesson view
// Attendance segmented toggle (spring thumb), drop-in sheet,
// credit-deduction on lesson done, AI note tidying.
// ============================================================
...
function AttToggle({ value, onChange }) {
...
function SessionAttendance({ onBack, onDone }) {
...
function LessonView({ onBack, onComplete }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--paper)' }}>
      ...
      <style>{`@keyframes r-float-up { from { opacity: 1; transform: translateY(0); } to { opacity: 0; transform: translateY(-22px); } } @keyframes r-fade { from { opacity: 0; } to { opacity: 1; } }`}</style>
    </div>
  );
}

function CoachHeader({ onBack, title, sub, weapon, live }) {
  return (
...
Object.assign(window, { SessionAttendance, LessonView, AttToggle, CoachHeader, ATT_STATES, ROSTER });
